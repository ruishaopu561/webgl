...
var VSHADER_SOURCE = `
    ...
    void main() {
      vec3 lightDirection = vec3(-0.35, 0.35, 0.87);                  //这个是我的光照的方向
      gl_Position = u_MvpMatrix * a_Position;                         //可以计算出来我的顶点位置在模型视图投影变换后的位置
...
function main() {
    ...
    readOBJFile('cube.obj', gl, model, 20, true);    //从这里往后的代码必须要理解；3objects.mqo
...
function readOBJFile(fileName, gl, model, scale, reverse) {
    ...
    request.onreadystatechange = function() {
        ...
        if (request.readyState === 4 && request.status !== 404) {
            ...
            /*
            * 实现目标1.：更立方体的每一个面都着不同的色
            * 思路：需要定义了六种材质，每一种材质的颜色各不相同
            * 实现目标2：在场景中同时加载出来两个立方体
            * 思路：可以先利用Blender软件建模生成模型，然后倒入进来即可
            * */
            ...
            var content = `
                ...
                mtllib cube.mtl
                o Cube
                v 1.000000 -1.000000 -1.000000        //在这里来设置立方体的顶点信息
                v 1.000000 -1.000000 1.000000
                v -1.000000 -1.000000 1.000000        //下底面（顺时针方向）
                v -1.000000 -1.000000 -1.000000

                v 1.000000 1.000000 -1.000000
                v 1.000000 1.000000 1.000001
                v -1.000000 1.000000 1.000000         //上顶面（顺时针方向）
                v -1.000000 1.000000 -1.000000

                usemtl Material1                    //在这里来使用材质（这里使用了Material这个材质）
                f 1 2 3 4         //底面
                usemtl Material2
                f 5 8 7 6         //上面
                usemtl Material3
                ...
                usemtl Material5
                f 5 1 4 8         //最后面
                usemtl Material6               //这里使用了Material.001这个材质
                f 1 5 6 2          //右面
                `;
            ...
            content = `
                ...
                mtllib cubes.mtl
                o 立方体
                v -1.364810 -0.595756 4.191937
                v -1.364810 1.404244 4.191937
                v -1.364810 -0.595756 2.191937
                v -1.364810 1.404244 2.191937
                v 0.635190 -0.595756 4.191937
                v 0.635190 1.404244 4.191937
                v 0.635190 -0.595756 2.191937
                v 0.635190 1.404244 2.191937

                v -0.090321 -1.142614 -2.992807
                v -0.090321 0.857386 -2.992807
                v -0.090321 -1.142614 -4.992807
                v -0.090321 0.857386 -4.992807
                v 1.909679 -1.142614 -2.992807
                v 1.909679 0.857386 -2.992807
                v 1.909679 -1.142614 -4.992807
                v 1.909679 0.857386 -4.992807
                ...
                f 5//2 6//2 2//2 1//2
                f 3//3 7//3 5//3 1//3
                f 15//1 16//1 14//1 13//1
                f 11//3 15//3 13//3 9//3
                f 16//4 12//4 10//4 14//4

                usemtl 材质.008
                f 13//2 14//2 10//2 9//2
                usemtl 材质.009
                f 11//5 12//5 16//5 15//5
                usemtl 材质.010
                f 1//6 2//6 4//6 3//6
                usemtl 材质.011
                f 3//5 4//5 8//5 7//5
                ...
                usemtl Material
                s off
                f 17//7 18//7 19//7 20//7
                f 21//8 24//8 23//8 22//8
                f 17//9 21//9 22//9 18//9
                f 18//10 22//10 23//10 19//10
                f 19//11 23//11 24//11 20//11
                f 21//12 17//12 20//12 24//12
                `;
            //再去调用我的解析文件的函数
                    ...
                    if (request.readyState == 4) {
                        if (request.status != 404) {
                            ...
                            var mtlContent = `
                                ...
                                # Material Count: 2
                                newmtl Material1
                                Ka 0.000000 0.000000 0.000000         //环境光
                                Kd 1.000000 0.000000 0.000000         //漫射色（也就是物体表面本来的颜色）
                                Ks 0.000000 0.000000 0.000000         //高光色
                                Ns 96.078431                          //指定了高光色颜色的权重
                                Ni 1.000000                           //表面的光学密度
                                d 1.000000                            //透明度
                                illum 0
